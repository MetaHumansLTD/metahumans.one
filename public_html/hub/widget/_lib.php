...
function mh_widget_require_auth(): array
{
    ...
    $tenantContextOk = false;
    if (function_exists('mh_apply_tenant_context')) {
        try {
            $tenantContextOk = (bool)mh_apply_tenant_context($tenantId);
        } catch (Throwable) {
            $tenantContextOk = false;
        }
    }
    ...
    return [
        ...
        'tenant_context_ok' => $tenantContextOk,
    ];
}
...
{
    $tenant = mh_widget_sanitize_id((string)($ctx['tenant_id'] ?? ''));
    $persona = mh_widget_sanitize_id((string)($ctx['persona_id'] ?? ''));
    $seed = $tenant . '|' . $persona;
    $hex = substr(hash('sha256', $seed), 0, 20);
    $room = 'mh_wgt_' . $hex;
    $room = preg_replace('/[^A-Za-z0-9_-]+/', '_', (string)$room);
    $room = substr($room, 0, 64);
    return $room !== '' ? $room : 'mh_wgt_default';
}
