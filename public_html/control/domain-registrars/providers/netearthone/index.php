...
if (ob_get_level() === 0) {
    ...
    define('MH_CTL_PROVIDERS_NETEARTHONE_OB_CLEANUP', true);
}
...
if (!is_file($cueBootstrapPath)) {
    ...
    if (defined('MH_CTL_PROVIDERS_NETEARTHONE_OB_CLEANUP')) { @ob_end_clean(); }
    if (!headers_sent()) {
        http_response_code(500);
        header('Content-Type: text/plain; charset=UTF-8', true);
    }
    echo "CUE bootstrap path is not available for registrar control dispatch.
";
    exit;
}
...
$integrationCandidates = [
    ...
    (defined('ROOT_PATH') ? (string)ROOT_PATH : '') . '/apps/domain-registrars/integrations/metahumans/control.php',
];
...
if (!$foundIntegration) {
    ...
    echo "Domain registrars control integration file is missing.
";
    exit;
}
