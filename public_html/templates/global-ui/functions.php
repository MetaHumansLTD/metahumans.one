...
function renderGlobalFooter($config = []) {
    ...
    echo '<script>(function(){try{var k=' . json_encode('mh_auth_event') . ';var bc=null;var lastTs=0;var scheduled=false;try{bc=new BroadcastChannel(' . json_encode('mh-auth') . ');}catch(e){}function scheduleReload(type,ts){ts=Number(ts||Date.now())||Date.now();if(ts<=lastTs)return;lastTs=ts;if(scheduled)return;scheduled=true;var delay=(type==="login")?(350+Math.floor(Math.random()*500)):50;setTimeout(function(){try{window.location.reload();}catch(e){}},delay);}function handle(payload){var d=payload&&typeof payload==="object"?payload:null;var t=d&&(d.type||d.event);if(t==="login"||t==="logout"){scheduleReload(t,d&&d.ts);}}if(bc){bc.onmessage=function(ev){handle(ev&&ev.data?ev.data:null);};}window.addEventListener("storage",function(ev){if(!(ev&&ev.key===k&&ev.newValue))return;try{handle(JSON.parse(ev.newValue));}catch(e){scheduleReload("login",Date.now());}});window.mhBroadcastAuthEvent=function(type){try{var payload={type:type,ts:Date.now()};try{localStorage.setItem(k,JSON.stringify(payload));}catch(e){};try{if(bc)bc.postMessage(payload);}catch(e){}}catch(e){}};}catch(e){}})();</script>';
