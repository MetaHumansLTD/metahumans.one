  ...
  var W = (function () {
    ...
    function ensureStyle() {
      ...
      st.textContent =
        ...
        ".mh-overlay-kv{font-size:12px;opacity:.9;word-break:break-word}" +
    ...
    function renderBody(body) {
      ...
      function updatePersonaPreview() {
        ...
        if (!url) {
          ...
          try { personaPreview.removeAttribute("src"); } catch (e) {}
          state.lastPreviewUrl = "";
          return;
        }
        try {
          if (!frame || frame.style.display === "none") personaPreview.style.display = "block";
          else personaPreview.style.display = "none";
        } catch (e) {}
        if (state.lastPreviewUrl === url) return;
        state.lastPreviewUrl = url;
        try { personaPreview.src = url; } catch (e) {}
      }

      function startSession() {
        var personaId = personaSel.value || "";
        var mode = modeSel.value || "auto";
        if (!personaId) { setStatus("Choose a persona first."); return; }
        if (personaId !== (state.lastPersistedPersona || "")) persistPersona(personaId);
        setStatus("Starting…");
        try { frame.src = ""; frame.style.display = "none"; } catch (e) {}
        try { if (personaPreview && personaPreview.src) personaPreview.style.display = "block"; } catch (e) {}
      ...
      modeSel.addEventListener("change", function () {
        if (isRealtimeMode()) { startSession(); return; }
        try { frame.src = ""; frame.style.display = "none"; } catch (e) {}
        updatePersonaPreview();
        setStatus("Ready.");
      });
